...
1. gfxpipe.ino: Arduino firmware listens for draw instructions (as defined in Adafruit_GFX) and paints them
2. server.js: Node.js server connects to Arduino via serial port, compiles and/or transmits encoded draw instructions to Arduino using rudimentary flow-control
3. localhost website: Interactive editor lets you write JavaScript in a web browser IDE (monaco-powered) to draw using standard Adafruit_GFX commands in realtime
...
- node.js (for streaming the generated UI to the Arduino) and working npm
- Adafruit_GFX.h library for Arduino
...
- Pull up http://localhost:8080/ in your browser
...
Instead of bothering with the interactive editor, you can craft raw instructions in any language to be transmitted to the arduino, and run `server.js --file=drawing.txt` mode where it will monitor for changes and send.
